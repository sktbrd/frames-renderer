...
async function parseFrame(url: string) {
  try {
    ...
    const imageMatch =
      html.match(
        /<meta\s+property="fc:frame:image"(?:\s+content="([^"]+)"|\s+name="([^"]+)")/i
      ) ||
      html.match(
        /<meta\s+property="og:image"(?:\s+content="([^"]+)"|\s+name="([^"]+)")/i
      );

    const titleMatch =
      html.match(
        /<meta\s+property="fc:frame:title"(?:\s+content="([^"]+)"|\s+name="([^"]+)")/i
      ) ||
      html.match(
        /<meta\s+property="og:title"(?:\s+content="([^"]+)"|\s+name="([^"]+)")/i
      ) ||
      html.match(/<title>([^<]+)<\/title>/i);

    const buttonsMatch = Array.from(
      html.matchAll(
        /<meta\s+property="fc:frame:button:(\d+)"(?:\s+content="([^"]+)"|\s+name="([^"]+)")/gi
      )
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `Processing button click for URL: ${frameUrl}, button: ${buttonIndex}`
    );
